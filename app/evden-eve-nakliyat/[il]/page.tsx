...
import { CheckIcon, ElevatorIcon } from "@/components/icons";
...
export async function generateMetadata({
  ...
  if (!city) return {};
  const content = getCityServiceContent(city, "evden-eve");
  return {
    title: content.title,
    description: content.metaDescription,
    alternates: { canonical: `/evden-eve-nakliyat/${city.slug}` },
  };
}
    ...
    <div className="container-page pt-8">
      <JsonLd
        data={serviceSchema({
          name: content.h1,
          description: content.metaDescription,
          url: `${SITE_URL}/evden-eve-nakliyat/${city.slug}`,
      ...
      <Breadcrumbs
        items={[
          ...
          { name: city.name, href: `/evden-eve-nakliyat/${city.slug}` },
        ]}
      />
      ...
      <section className="section-padding">
        ...
        <div className="mt-10">
          <CityLinkGrid cities={content.relatedCities} basePath="/evden-eve-nakliyat" />
        </div>
      ...
      <section className="pb-20 sm:pb-28">
        <CTASection
          title={`${city.name}'da Taşınmaya Hazır mısınız?`}
